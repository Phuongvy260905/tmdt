...
const OrdersModule = {
    ...
    // 3. Hàm render dữ liệu (Đã sửa để nhận tham số data khi lọc)
    async loadOrders(data = null) {
        ...
        // Nếu data null (lần đầu load), gọi hàm fetch. Nếu có data (đã lọc), dùng luôn.
        let orders = data;
        if (!orders) {
    ...
    filterOrders() {
        ...
        if (filterValue === 'all') {
            ...
            const filtered = this.allOrders.filter(order => order.status === filterValue);
